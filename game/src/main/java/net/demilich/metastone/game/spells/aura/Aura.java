...
public class Aura extends Enchantment implements HasDesc<AuraDesc> {
	...
	private void constructAura(AuraDesc desc) {
		setDesc(desc);
		var extraTriggers = (EventTriggerDesc[]) getDesc().getOrDefault(AuraArg.TRIGGERS, EMPTY_TRIGGERS);
		for (var trigger : ObjectArrays.concat(getDefaultTriggers(), extraTriggers, EventTriggerDesc.class)) {
