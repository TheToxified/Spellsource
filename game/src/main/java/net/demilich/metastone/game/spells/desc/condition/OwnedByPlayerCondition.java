...
public final class OwnedByPlayerCondition extends Condition {
	...
	protected boolean isFulfilledForTarget(GameContext context, Player player, Entity source, Entity target, boolean invert, TargetPlayer targetPlayer) {
		switch (targetPlayer) {
			case ACTIVE:
				return context.getActivePlayerId() == player.getId() != invert;
			case INACTIVE:
				return (context.getActivePlayerId() != player.getId()) != invert;
			case BOTH:
				return false;
			case EITHER:
				return true;
			case OPPONENT:
				return (target != null && target.getOwner() != player.getId()) != invert;
			case SELF:
				return (target != null && target.getOwner() == player.getId()) != invert;
			case PLAYER_1:
				return (target != null && target.getOwner() == GameContext.PLAYER_1) != invert;
			case PLAYER_2:
				return (target != null && target.getOwner() == GameContext.PLAYER_2) != invert;
			default:
				break;
