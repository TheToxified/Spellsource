...
public class GameLogic implements Cloneable, Serializable, IdFactory {
	...
	public void addEnchantment(Player player, Enchantment enchantment, Entity effectSource, Entity host) {
		...
		if (!enchantment.hasPersistentOwner() || enchantment.getOwner() == Entity.NO_OWNER) {
	...
	public void fight(Player player, Actor attacker, Actor defender, PhysicalAttackAction sourceAction) {
		...
		var damageDealtToAttacker = applyDamageToActor(attacker, defenderDamage, player, defender, true, DamageTypeEnum.PHYSICAL);
		var damageDealtToDefender = applyDamageToActor(defender, attackerDamage, player, attacker, true, DamageTypeEnum.PHYSICAL);
	...
	public HealingResult heal(Player player, Actor target, int healing, Entity source, boolean applyHealingBonus) {
		...
		if (excess > 0) {
			fireGameEvent(new ExcessHealingEvent(context, player.getId(), target, excess));
	...
	public void startGameForPlayer(Player player) {
		...
		fireGameEvent(new GameStartEvent(context, player.getId()));
	}
	...
	public void innerChangeOwner(Entity target, int newOwnerId) {
		...
		for (var trigger : triggers) {
			...
			addEnchantment(context.getPlayer(newOwnerId), (Enchantment) trigger, target, target);
		}
	}
	...
	public void removeAttribute(Entity entity, Entity source, Attribute attr) {
		...
		var sourcePlayerId = source == null ? -1 : source.getOwner();
		if (attr == Attribute.STEALTH) {
			fireGameEvent(new LoseStealthEvent(context, entity, sourcePlayerId));
		}

		if (attr == Attribute.DEFLECT) {
			fireGameEvent(new LoseDeflectEvent(context, entity, entity.getId(), sourcePlayerId));
		}

		if (attr == Attribute.DIVINE_SHIELD) {
			fireGameEvent(new LoseDivineShieldEvent(context, entity, entity.getOwner(), sourcePlayerId));
		}
	...
	@Suspendable
	public void removeEnchantments(Entity entity, final boolean removeAuras, final boolean keepSelfCardCostModifiers, final boolean removeAddedAftermaths) {
		removeEnchantments(entity, trigger -> {
			...
			if (!removeAddedAftermaths && trigger instanceof Aftermath) {
				return false;
			}
			return true;
		});
	...
	public OpenerAction[] resolveOpeners(int playerId, Actor actor) {
		...
		if (context.getCurrentAction() != null
				&& context.getCurrentAction() instanceof HasOpener
				&& ((HasOpener) context.getCurrentAction()).getOpener() != null
				&& Objects.equals(context.getCurrentAction().getSourceReference(), actor.getSourceCard().getReference())) {
			var openerDesc = ((HasOpener) context.getCurrentAction()).getOpener();
	 ...
	 *                      enchantments.
	 * @param source        The {@link Card} or {@link Entity} responsible for summoning this minion. If this is a {@link
	 *                      EntityType#CARD} of {@link CardType#MINION} and the source has the attribute {@link
	...
	public boolean summon(int playerId, @NotNull Minion minion, @NotNull Entity source, int index, boolean resolveOpener) {
		...
		try {
			...
			if (!minion.hasAttribute(Attribute.PERMANENT)) {
				...
				fireGameEvent(new BeforeSummonEvent(context, minion, source, false, null));
				// Might transform
	...
	public List<Enchantment> addEnchantments(@NotNull Player player, @NotNull Entity effectSource, @NotNull Card enchantmentSource, @NotNull Entity host, boolean force) {
		...
		var iterator = desc.getEnchantmentDescs().iterator();
	...
	public void questTriggered(Player player, Quest quest) {
		...
		if (quest.isInPlay()) {
			...
			fireGameEvent(new QuestSuccessfulEvent(context, quest.getSourceCard(), player.getId()));
		}
	}
	...
	public void discoverCard(int playerId) {
		...
		fireGameEvent(new DiscoverEvent(context, playerId));
	}
			...
			for (var trigger : triggers) {
				...
				try {
					...
					if (event.getEventType() == com.hiddenswitch.spellsource.client.models.GameEvent.EventTypeEnum.TURN_END
							&& trigger.oneTurnOnly()) {
						if (!trigger.interestedIn(com.hiddenswitch.spellsource.client.models.GameEvent.EventTypeEnum.TURN_START)
